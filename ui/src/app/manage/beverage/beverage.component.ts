...
export class ManageBeverageComponent implements OnInit {
  ...
  beverageBatches: Record<string, Batch[]> = {};

  userInfo!: UserInfo;

  requiredIfImageTransitionsEnabled(comp: ManageBeverageComponent): ValidatorFn {
    return (control: AbstractControl): ValidationErrors | null => {
      const imageTransitionsEnabled = _.get(
        comp.modifyBeverage.editValues,
            ...
            this.dataService.getBeverages(true).subscribe({
              next: (beverages: Beverage[]) => {
                ...
                _.forEach(beverages, beverage => {
                  ...
                  this.dataService
                    .getBeverageBatches(beverage.id, true, this.showArchivedBatches)
                    .subscribe({
                      next: (batches: Batch[]) => {
  ...
  addBeverage(): void {
    ...
    const data: any = { type: this.defaultType, meta: {} };
  ...
  createBeverage(): void {
    ...
    if (this.modifyBeverage.imageTransitions) {
      ...
      for (const i of this.modifyBeverage.imageTransitions) {
  ...
  get beverageChanges(): any {
    const changes = _.cloneDeep(this.modifyBeverage.changes);
  ...
  filter(sort?: Sort) {
    ...
    let filteredData: Beverage[] = this.beverages;
  ...
  get areImageTransitionsValid(): boolean {
    if (
      _.isNil(this.modifyBeverage.editValues) ||
      _.isNil(this.modifyBeverage.imageTransitions) ||
      !this.modifyBeverage.imageTransitionsEnabled
    ) {
      return true;
    }

    for (const imageTransition of this.modifyBeverage.imageTransitions) {
      if (
  ...
  removeImageTransitionItemFromList(imageTransition: ImageTransition): void {
    ...
    if (
      ...
      isNilOrEmpty(this.modifyBeverage) ||
      !this.modifyBeverage.imageTransitions
    ) {
      return;
    }
  ...
  private _processAffectedTaps(taps: Tap[], index: number, next: () => void): void {
    ...
    const hasKegtronMeta = meta != null && meta.deviceId != null && typeof meta.portNum === 'number';
  ...
  toggleArchivedBatches(): void {
    ...
    if (this.editingBeverage && this.modifyBeverage && this.modifyBeverage.id) {
      this.dataService
        .getBeverageBatches(this.modifyBeverage.id, true, this.showArchivedBatches)
        .subscribe({
          next: (batches: Batch[]) => {
