...
class PlaatoKeg(TapMonitorBase):
    ...
    async def get(self, data_key, monitor_id=None, monitor=None, meta=None, db_session=None, **kwargs):
    ...
    async def get_all(self, monitor_id=None, monitor=None, meta=None, db_session=None, **kwargs):
        data = await self._get_data(monitor_id, monitor, meta, db_session)
    ...
    async def discover(self, db_session=None, **kwargs):
        if not db_session:
            with async_session_scope(self.config) as db_session:
                return await self.discover(db_session, **kwargs)
    ...
    async def _get_data(self, monitor_id=None, monitor=None, meta=None, db_session=None):
    ...
    async def _get(self, db_session, device_id):
        if not db_session:
