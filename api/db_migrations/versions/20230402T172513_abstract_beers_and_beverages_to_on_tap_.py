...
def upgrade():
    ...
    op.execute(
        "insert into batches (tap_id, beer_id, beverage_id, tapped_on) (select id, beer_id, beverage_id, now() from taps where not (beer_id is null and beverage_id is null))"
    )
    ...
    op.execute(
        "update batches set external_brewing_tool=b.external_brewing_tool, external_brewing_tool_meta=b.external_brewing_tool_meta from (select id, external_brewing_tool, external_brewing_tool_meta from beers) AS b where batches.beer_id=b.id"
    )
...
def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.execute(
        "update beers set external_brewing_tool=b.external_brewing_tool, external_brewing_tool_meta=b.external_brewing_tool_meta from (select distinct beer_id, external_brewing_tool, external_brewing_tool_meta from batches) AS b where beers.id=b.beer_id"
    )
    ...
    op.execute(
        "update taps set beer_id=sq.beer_id, beverage_id=sq.beverage_id from (select ot.id, b.beer_id, b.beverage_id from on_tap as ot join batches as b on ot.batch_id=b.id) AS sq where taps.on_tap_id=sq.id"
    )
    ...
    op.execute(
        "update beers set brew_date=sq.brew_date, keg_date=sq.keg_date from (select id, brew_date, keg_date, beer_id from batches) AS sq where beers.id=sq.beer_id"
    )
    ...
    op.execute(
        "update beverages set brew_date=sq.brew_date, keg_date=sq.keg_date from (select id, brew_date, keg_date, beverage_id from batches) AS sq where beverages.id=sq.beverage_id"
    )
