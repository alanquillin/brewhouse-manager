...
class BeerResourceMixin(ResourceMixinBase):
    ...
    def update(self, beer_id, data, db_session=None):
        if not db_session:
            with session_scope(self.config) as db_session:
                return self.update(beer_id, data, db_session=db_session)
        
        self.logger.debug("Updating beer %s with: %s", beer_id, data)
        BeersDB.update(db_session, beer_id, **data)
    
    def transform_response(self, beer, db_session=None, **kwargs):
        if not beer:
            return beer
        ...
        if tool_type:
            ...
            if refresh_data:
                self.logger.info("Refreshing data from %s for %s.  Reason: %s", tool_type, beer.id, refresh_reason)
                tool = get_external_brewing_tool(tool_type)
                ex_details = tool.get_details(beer=beer)
                if ex_details:
                    self.logger.debug("Extended beer details: %s, updateing database", ex_details)
                    self.update(beer.id, {"external_brewing_tool_meta": {**meta, "details": ex_details}}, db_session=db_session)
                else:
                    self.logger.warning("There and error or no details from %s for %s", tool_type, {k:v for k,v in meta.items() if k != "details"})
